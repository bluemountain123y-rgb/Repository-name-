...
import quiz_ui as ui # 先ほどのファイルをインポート

st.set_page_config(page_title="コンクリート主任技士 試験対策", page_icon="🏗️", layout="centered")

# --- 1. ユーザー認証 ---
credentials = {"usernames": {"admin": {"name": "管理者", "password": "concrete2026"}, "user01": {"name": "テストユーザー", "password": "guest456"}}}
...
if st.session_state["authentication_status"]:
    ...
    if base_data:
        ...
        selected_cat = st.sidebar.selectbox("📂 カテゴリ選択", cat_options).split(" (")[0]
    ...
    if not filtered_data:
        ...
        if mode == "見直しリスト":
            ...
            # 通常学習モード
            target_total = st.sidebar.slider("🎯 目標出題数", 5, 50, 10)
            if st.session_state.count >= target_total:
                st.success("目標達成！記録をリセットして再挑戦できます。")
            else:
